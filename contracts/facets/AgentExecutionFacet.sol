...
contract AgentExecutionFacet {
    ...
    event ExecutionConfigUpdated(address userStateINFT, address userStateLedger);
    ...
    function setExecutionConfig(address _userStateINFT, address _userStateLedger)
        external
        onlyAdmin
    {
    ...
    function getExecutionConfig() external view returns (address inft, address ledger) {
        LibAgentExecutionStorage.Layout storage s = LibAgentExecutionStorage.layout();
    ...
    function request(
        ...
        return _createRequest(msg.sender, user, tokenId, inputPointer, inputType, runId, stepIndex);
    }
    ...
    function userRequest(
        ...
        // Verify msg.sender owns the token. We call ownerOf on the configured iNFT.
    ...
    function _createRequest(
        ...
        return _createRequestInner(CreateReqParams({
            workflow: workflow,
            user: user,
            tokenId: tokenId,
    ...
    function _createRequestInner(CreateReqParams memory p) internal returns (bytes32 requestKey) {
        ...
        requestKey = LibAgentExecutionStorage.requestKey(p.workflow, p.runId, p.stepIndex);
        LibAgentExecutionStorage.RequestRecord storage r = s.requests[requestKey];
        if (r.status != LibAgentExecutionStorage.RequestStatus.NONE) revert WrongStatus();
    ...
    function acknowledge(bytes32 requestKey) external onlyWorker {
        LibAgentExecutionStorage.RequestRecord storage r =
            LibAgentExecutionStorage.layout().requests[requestKey];
        if (r.status == LibAgentExecutionStorage.RequestStatus.NONE) revert UnknownRequest();
        if (r.status != LibAgentExecutionStorage.RequestStatus.CREATED) revert WrongStatus();
    ...
    function getRequest(bytes32 requestKey)
        external
        view
        returns (LibAgentExecutionStorage.RequestRecord memory)
    {
        return LibAgentExecutionStorage.layout().requests[requestKey];
    }

    function getRequestKeysForUser(address user) external view returns (bytes32[] memory) {
        return LibAgentExecutionStorage.layout().userRequests[user];
    }

    function getAllRequestKeys(uint256 cursor, uint256 limit)
        external
        view
    ...
    function _supportsInput(LibAgentManifestStorage.AgentManifest storage m, bytes32 t)
        internal
        view
        returns (bool)
    {
